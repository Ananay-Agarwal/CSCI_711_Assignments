...
class Raytracer:
    def __init__(self):
        ...
        self.background_color = np.array([0, 0, 0], dtype=float)   # black (no extra light)
        self.camera = None
        ...
        self.kdtree = None
        self.lights = []
        self.ambient_light = np.array([0.0, 0.0, 0.0], dtype=float)   # will be set later
        self.max_depth = 8
        self.radiance_image = None
    ...
    def illuminate(self, ray, depth):
        ...
        return color   # no clamping – HDR radiance
    ...
    def render(self):
        ...
        for i, row in enumerate(self.focal_plane):
            ...
            for j, pixel_world in enumerate(row):
                ...
                radiance = self.illuminate(ray, depth=1)
                img_row.append(radiance)   # store raw RGB radiance (float)
            self.radiance_image.append(img_row)
        end = time.perf_counter()
        print(f"done in {end - start:.6f} seconds.")
    ...
    @staticmethod
    def luminance(rgb):
        """Compute luminance from RGB (standard sRGB weights)."""
        return 0.27 * rgb[0] + 0.67 * rgb[1] + 0.06 * rgb[2]

    def ward_tone_mapping(self, hdr_image, Ldmax, adaptation_luminance=None):
        """
        Ward's tone reproduction operator.
        hdr_image: list of list of [R,G,B] floats (radiance)
        Ldmax: maximum display luminance (cd/m^2)
        adaptation_luminance: if None, use log-average luminance; else given value.
        Returns an 8-bit RGB image (list of list of (R,G,B) in 0..255).
        """
        height = len(hdr_image)
        width = len(hdr_image[0]) if height > 0 else 0

        # Compute log-average luminance if needed
        if adaptation_luminance is None:
            log_sum = 0.0
            N = height * width
            delta = 1e-6
            for row in hdr_image:
                for pix in row:
                    L = self.luminance(pix)
                    log_sum += np.log(delta + L)
            Lwa = np.exp(log_sum / N)
        else:
            Lwa = adaptation_luminance

        # Scale factor
        sf_numer = 1.219 + (Ldmax / 2.0) ** 0.4
        sf_denom = 1.219 + Lwa ** 0.4
        sf = (sf_numer / sf_denom) ** 2.5

        # Apply scaling and device model (divide by Ldmax)
        result_img = []
        for row in hdr_image:
            new_row = []
            for rgb in row:
                scaled = sf * rgb
                target = scaled   # target luminance in [0, Ldmax]
                final = target / Ldmax   # now in [0,1]
                final_rgb = np.clip(final * 255, 0, 255).astype(np.uint8)
                new_row.append(tuple(final_rgb))
            result_img.append(new_row)
        return result_img

    def reinhard_tone_mapping(self, hdr_image, Ldmax, key_value=None):
        """
        key_value : None -> use log-average luminance
                    float -> use that constant value for all pixels
                    array (HxW) -> per-pixel key values
        """
        height = len(hdr_image)
        width = len(hdr_image[0]) if height > 0 else 0

        if key_value is None:
            # Compute log-average (original behaviour)
            log_sum = 0.0
            N = height * width
            delta = 1e-6
            for row in hdr_image:
                for pix in row:
                    L = self.luminance(pix)
                    log_sum += np.log(delta + L)
            key = np.exp(log_sum / N)
        elif isinstance(key_value, (float, int)):
            key = float(key_value)
        else:
            # Assume key_value is a 2D array of same dimensions
            key = key_value

        a = 0.18  # Zone V gray

        result_img = []
        for i, row in enumerate(hdr_image):
            new_row = []
            for j, rgb in enumerate(row):
                if isinstance(key, np.ndarray):
                    ...
                    k = key
                scaled = (a / k) * rgb
                reflectance = scaled / (1.0 + scaled)
                target = reflectance * Ldmax
                final = target / Ldmax  # = reflectance
                final_rgb = np.clip(final * 255, 0, 255).astype(np.uint8)
                new_row.append(tuple(final_rgb))
            result_img.append(new_row)
        return result_img